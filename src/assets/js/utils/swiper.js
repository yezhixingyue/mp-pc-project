...
const css = (el, attr) => {
  ...
  const getOrSet = type(attr) != 'object';
 ...
 */
const tweenMove = ({
  el, type, to, time, callBack, callIn,
}) => {
  let t = 0;
  const b = {};
  const c = {};
  const d = Math.round(time / 16.7);
 ...
 */
const swiper = ({
  wrap, dir = 'y', start, move, end, over, toTop, toEnd, backOut = 'back', scrollBar = true,
}) => {
    ...
    if (isDir[dir]) {
      if (backOut == 'none') {
        ...
        if (targetEle[dir] > 0) {
          ...
          const dis = minDistance[dir] - targetEle[dir];
